...
class KiFilters {
  def filters = {
    accessControlCheck(controller: '*', action: '*') {
      before = {
        if (grailsApplication.config.security.ki.annotationdriven.enabled) {
          // Check that the current user is authenticated and has permission to access the current action.
