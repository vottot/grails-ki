...
/**
 * Installs a Ki template into the current project, where the
...
/**
 * Installs a Ki template into the current project, with optional
 * post-processing.
...
installTemplateEx = { String artefactName, String artefactPath, String templatePath, String templateName, Closure c ->
    ...
    templateFile = "${kiPluginDir}/src/templates/artifacts/${templatePath}/${templateName}"
    if (!new File(templateFile).exists()) {
        Ant.echo("[Ki plugin] Error: src/templates/artifacts/${templatePath}/${templateName} does not exist!")
        return
    }
